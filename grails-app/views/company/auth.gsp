  ...
  <head>
    ...
    <g:javascript>
      ...
      function processResponse(e){
        ...
        var sErrorMsg = '';
        ['su_username','su_login','su_password','su_password2'].forEach(function(ids){
          if($(ids))
        ...
        if(e.responseJSON.errorcode.length){
          e.responseJSON.errorcode.forEach(function(err){
            switch (err) {
              case 1: sErrorMsg+='<li>${message(code:"error.blank.message",args:["Имя"])}</li>'; $('su_username').addClassName('red'); break;
              case 2: sErrorMsg+='<li>${message(code:"error.blank.message",args:["Email"])}</li>'; $('su_login').addClassName('red'); break;
              case 3: sErrorMsg+='<li>${message(code:"error.incorrect.message",args:["Email"])}</li>'; $('su_login').addClassName('red'); break;
              case 4: sErrorMsg+='<li>${message(code:"error.not.unique2.message",args:["Пользователь", "Email"])}</li>'; $('su_login').addClassName('red'); break;
              ...
              case 7: sErrorMsg+='<li>Некорректный пароль. Пароль не менее ${Tools.getIntVal(Dynconfig.findByName('customer.passwordlength')?.value,7)} знаков из больших и маленьких латинских букв и цифр</li>'; $("su_password").addClassName('red'); break;
          ...
          location.assign('${createLink(controller:'company',action:'index',id:partner.id)}');
      }
    </g:javascript>
  </head>
  <body>
    ...
    <div id="loginDiv" class="grid_6 padtop" style="max-height:201px">
      ...
      <g:form url="[controller:'company', action:'login', params:[linkname:partner.linkname]]" method="post" autocomplete='off'>
        ...
        <span>
          <g:link class="button" style="margin:0 15px;padding-top:10px;line-height: 18px;" url="${[controller:'company', action:'restore', params:[linkname:partner.linkname]]}">Склероз? &nbsp;<i class="icon-angle-right icon-large"></i></g:link>
        </span>
    ...
    <div id="signUpDiv" class="grid_6 padtop" style="display:none;max-height:201px">
      ...
      <g:formRemote name="signupForm" url="${[controller:'company', action:'signup', params:[linkname:partner.linkname]]}" method="post" autocomplete='off' onSuccess="processResponse(e)">
